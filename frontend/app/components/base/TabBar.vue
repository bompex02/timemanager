  ...
  <div class="flex items-center">
    ...
    >
      <TabsList as-child>
        <div
          ref="bar"
          class="relative grid flex-1 snap-x snap-proximity grid-flow-col scroll-smooth scrollbar-hidden"
          style="grid-auto-columns: 1fr"
          :class="dense ? 'gap-x-2' : 'gap-x-4'"
          tabindex="-1"
        >
          <TabsIndicator class="absolute bottom-0 left-0 h-0.5 w-(--reka-tabs-indicator-size) translate-x-(--reka-tabs-indicator-position) bg-text-100 transition-[translate] duration-400" />
          <TabsTrigger
            v-for="(tab, index) in tabsAsObjects"
            :key="tab.text"
            :value="index"
            :disabled="tab.disabled"
            class="snap-start border-b-2 text-text rounded-t-lg border-zinc-400 px-2 whitespace-nowrap transition-[font-weight] duration-400
